  ...
  useEffect(() => {
    if (isEditing && editableSections.length === 0) {
      const converted = sections.map((section) => {
        ...
        // Add schedule as bullets module
        if (section.schedule) {
          modules.push({
            id: `${section.title}-schedule-${Date.now()}`,
            type: 'bullets',
            content: { title: 'Schedule', items: section.schedule },
            column: 'left'
          });
        }

        // Add session details as text modules
        if (section.sessionDetails) {
          section.sessionDetails.forEach((detail: string, index: number) => {
            const [title, description] = detail.split('\n');
            modules.push({
              id: `${section.title}-detail-${index}-${Date.now()}`,
              type: 'quote',
        ...
        // Add focus areas as quote modules
        if (section.focusAreas) {
          section.focusAreas.forEach((area: any, index: number) => {
            const content = `${area.description}\n\n${area.points.map((point: string) => `• ${point}`).join('\n')}`;
            modules.push({
              id: `${section.title}-focus-${index}-${Date.now()}`,
              type: 'quote',
              content: { title: area.title, text: content },
              column: 'full'
            });
          });
        }

        // Add learnings as quote modules
        if (section.learnings) {
          section.learnings.forEach((learning: string, index: number) => {
            const [title, description] = learning.split(': ');
            modules.push({
              id: `${section.title}-learning-${index}-${Date.now()}`,
              type: 'quote',
              content: { title, text: description },
              column: 'full'
            });
        ...
        if (section.image || section.sectionImage || section.fullWidthImage) {
          modules.push({
            id: `${section.title}-main-image-${Date.now()}`,
            type: 'image',
            content: { 
        ...
        // Add workshop images
        if (section.workshopImages) {
          section.workshopImages.forEach((image: string, index: number) => {
            modules.push({
              id: `${section.title}-workshop-${index}-${Date.now()}`,
              type: 'image',
              content: { src: image, alt: `Workshop image ${index + 1}` },
              column: 'right'
            });
          });
        }
        ...
        if (section.additionalImages) {
          section.additionalImages.forEach((image: string, index: number) => {
            modules.push({
              id: `${section.title}-additional-${index}-${Date.now()}`,
              type: 'image',
              content: { src: image, alt: `Additional image ${index + 1}` },
              column: 'right'
            });
          });
        }
